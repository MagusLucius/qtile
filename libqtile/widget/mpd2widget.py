...
class Mpd2(base.ThreadPoolText):
    ...
    def formatter(self, status, current_song):
        ...
        # progress_color = "030306"
        formatted = fmt.format(**song_info)
        # try:
        #     elapsed, total = status['time'].split(':')
        #     percent = float(elapsed) / float(total)
        #     progress = int(percent * len(formatted))
        #     formatted = '<span background="{0}">{1}</span>{2}'.format(
        #         utils.hex(str(progress_color)),
        #         pangocffi.markup_escape_text(formatted[:progress]),
        #         pangocffi.markup_escape_text(formatted[progress:]),
        #     )
